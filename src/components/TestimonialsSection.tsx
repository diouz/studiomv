...
const TestimonialsSection: React.FC = () => {
  ...
  const renderStars = (rating: number) => {
    return Array.from({ length: 5 }, (_, index) => (
      <Star
        key={index}
        className={`w-5 h-5 ${index < rating ? 'text-amber-400 fill-current' : 'text-stone-400'
          }`}
      />
    ));
  };
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <ImmersiveTransitions direction="up" delay={600} duration={1000}>
          <div className="max-w-4xl mx-auto">
            <div className="bg-white/5 backdrop-blur-md border border-white/10 rounded-3xl p-8 md:p-12 relative transition-all duration-500 hover:bg-white/10 hover:border-white/20 hover:shadow-2xl hover:shadow-amber-500/10 hover:scale-[1.02]">
              {/* Quote Icon */}
              ...
              <div className="text-center">
                ...
                <div className="flex items-center justify-center gap-4">
                  ...
                  <div className="flex-shrink-0">
                    {currentTestimonial.clientAvatar && typeof currentTestimonial.clientAvatar === 'object' ? (
                      <img
                        src={currentTestimonial.clientAvatar.url}
                        alt={currentTestimonial.clientName}
                        className="w-16 h-16 rounded-full object-cover border-2 border-amber-400/30 transition-transform duration-300 group-hover:scale-110"
                      />
                    ) : (
